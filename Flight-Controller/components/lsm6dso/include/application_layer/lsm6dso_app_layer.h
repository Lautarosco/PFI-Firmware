...
#define LSM6DSO_GYRO_OFFSET_SAMPLES_DEFAULT     1000
...
typedef struct lsm6dso_gyro_params {
    ...
    unsigned int offset_samples;    /* Total samples to be taken in order to compute Gyroscope (x, y, z) offsets */
...
typedef struct lsm6dso_private {
    ...
    float gyro_x_off;                                   /* Gyroscope x-axis offset */
    float gyro_y_off;                                   /* Gyroscope y-axis offset */
    float gyro_z_off;                                   /* Gyroscope z-axis offset */
} lsm6dso_private_t;
...
typedef struct lsm6dso {
    ...
    /**
     * @brief Use this function if Accelerometer and Gyroscope have the same sampling frequency (ODR)
     * 
     * @param lsm: Pointer to lsm6dso_t struct
     * @retval
     *      - ESP_OK if success
     */
    esp_err_t (*measure)(lsm6dso_t *lsm);
    esp_err_t (*measure_acc)(lsm6dso_t *lsm);
    esp_err_t (*measure_gyro)(lsm6dso_t *lsm);
    esp_err_t (*measure_temp)(lsm6dso_t *lsm);
} lsm6dso_t;
