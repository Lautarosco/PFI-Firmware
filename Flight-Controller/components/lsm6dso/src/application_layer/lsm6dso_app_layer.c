...
esp_err_t lsm6dso_init(lsm6dso_t *lsm, lsm6dso_params_t lsm_params) {
    ...
    /* Compute Gyroscope (x, y, z) offsets */
    /* First check if user-defined parameter is valid else use default value */
    unsigned int gyro_off_samples = 0;
    if(lsm_params.gyro.offset_samples <= 0) {
        gyro_off_samples = LSM6DSO_GYRO_OFFSET_SAMPLES_DEFAULT;
    } else {
        gyro_off_samples = lsm_params.gyro.offset_samples;
    }

    /* Initialize totalizer variables */
    float dummy_gyro_x = 0.0f;
    float dummy_gyro_y = 0.0f;
    float dummy_gyro_z = 0.0f;
    ESP_LOGI(
        lsm6dso_app_layer_tag,
        "{Function %s in line %d}: Estimated time for <%u> samples: %.4f s"
        , __func__, __LINE__,
        gyro_off_samples, gyro_off_samples * (1.0f / gyro_freq_hz[lsm_params.gyro.odr])
    );

    /* Measure Gyroscope N times */
    for(int i = 0; i < gyro_off_samples; i++) {
        esp_err_t ret = lsm6dso_measure(lsm);
        if(ret != ESP_OK) {
            ESP_LOGE(lsm6dso_app_layer_tag, "{Function %s in line %d}: Calibrate Gyroscope --> FAILED", __func__, __LINE__);
            return ESP_FAIL;
        }

        dummy_gyro_x += lsm->gyro.x;
        dummy_gyro_y += lsm->gyro.y;
        dummy_gyro_z += lsm->gyro.z;

        vTaskDelay(pdMS_TO_TICKS(1000.0f / gyro_freq_hz[lsm_params.gyro.odr]));
    }

    /* Get average values from above measurements */
    lsm->_private.gyro_x_off = dummy_gyro_x / gyro_off_samples;
    lsm->_private.gyro_y_off = dummy_gyro_y / gyro_off_samples;
    lsm->_private.gyro_z_off = dummy_gyro_z / gyro_off_samples;
    ESP_LOGI(
        lsm6dso_app_layer_tag,
        "{Function %s in line %d}: Gyro_x_off: %.2f °/s, Gyro_y_off: %.2f °/s, Gyro_z_off: %.2f °/s"
        , __func__, __LINE__,
        lsm->_private.gyro_x_off, lsm->_private.gyro_y_off, lsm->_private.gyro_z_off
    );
...
void Lsm6dso(lsm6dso_t *lsm) {
    ...
    lsm->measure_acc = lsm6dso_measure_acc;
    lsm->measure_gyro = lsm6dso_measure_gyro;
    lsm->measure_temp = lsm6dso_measure_temp;
}
