...
const char *bmp390_hwl_tag = "[BMP390_HW_LAYER]";
...
typedef struct bmp390_reg_modes {
    ...
    uint8_t total_bits;                                             /* Total bits used for this mode */
    uint8_t starting_bit_pos;                                       /* Starting bit position */
    const char* (*get_name_func)(unsigned int mode_value);          /* [bmp390_get_<register>_name] function */
} bmp390_reg_modes_t;

static const bmp390_reg_modes_t bmp390_reg_modes_arr[] = {
    {.reg = BMP390_CONFIG_RW_REG,   .mode_name = "iir_filter",  .total_bits = 3, .starting_bit_pos = BMP390_CONFIG_IIR_BITS,         .get_name_func = bmp390_get_iir_coef_name},
    {.reg = BMP390_ODR_RW_REG,      .mode_name = "odr_sel",     .total_bits = 5, .starting_bit_pos = BMP390_ODR_ODR_SEL_BITS,        .get_name_func = bmp390_get_odr_sel_name},
    {.reg = BMP390_OSR_RW_REG,      .mode_name = "osr_p",       .total_bits = 3, .starting_bit_pos = BMP390_OSR_P_BITS,              .get_name_func = bmp390_get_osr_press_name},
    {.reg = BMP390_OSR_RW_REG,      .mode_name = "osr_t",       .total_bits = 3, .starting_bit_pos = BMP390_OSR_T_BITS,              .get_name_func = bmp390_get_osr_temp_name},
    {.reg = BMP390_PWR_CTRL_RW_REG, .mode_name = "mode",        .total_bits = 2, .starting_bit_pos = BMP390_PWR_CTRL_MODE_BITS,      .get_name_func = bmp390_get_pwr_mode_name},
    {.reg = BMP390_PWR_CTRL_RW_REG, .mode_name = "press_en",    .total_bits = 1, .starting_bit_pos = BMP390_PWR_CTRL_PRESS_EN_BIT,   .get_name_func = bmp390_get_press_status},
    {.reg = BMP390_PWR_CTRL_RW_REG, .mode_name = "temp_en",     .total_bits = 1, .starting_bit_pos = BMP390_PWR_CTRL_TEMP_EN_BIT,    .get_name_func = bmp390_get_temp_status},
    {.reg = BMP390_IF_CONF_RW_REG,  .mode_name = "spi3",        .total_bits = 1, .starting_bit_pos = BMP390_IF_CONF_SPI3_BIT,        .get_name_func = bmp390_get_spi_mode_name},
    {.reg = BMP390_IF_CONF_RW_REG,  .mode_name = "i2c_wdt_en",  .total_bits = 1, .starting_bit_pos = BMP390_IF_CONF_I2C_WDT_EN_BIT,  .get_name_func = bmp390_get_i2c_wdt_en_status},
    {.reg = BMP390_IF_CONF_RW_REG,  .mode_name = "i2c_wdt_sel", .total_bits = 1, .starting_bit_pos = BMP390_IF_CONF_I2C_WDT_SEL_BIT, .get_name_func = bmp390_get_i2c_wdt_tout_name},
};
...
void bmp390_hwl_get_mode_val(device_interface_t dev_iface, uint8_t reg_addr, uint8_t starting_bit_pos) {
    for(int i = 0; i < ((sizeof(bmp390_reg_modes_arr)) / (sizeof(bmp390_reg_modes_arr[0]))); i++) {
        ...
        if((bmp390_reg_modes_arr[i].starting_bit_pos == starting_bit_pos) && (bmp390_reg_modes_arr[i].reg == reg_addr)) {
            /* Read DATA register content and check for errors */
            uint8_t reg_value = 0;

            if((dev_iface.read_bytes(dev_iface.master_cfg, dev_iface.dev_cfg, reg_addr, &reg_value, 1, dev_iface.iface_sel) != ESP_OK) || (bmp390_hwl_err(dev_iface) != ESP_OK)) {
                ESP_LOGE(bmp390_hwl_tag, "{Function <%s> in line %d}: Read bytes --> FAILED", __func__, __LINE__);
            }

            uint8_t mask = ((1U << bmp390_reg_modes_arr[i].total_bits) - 1) << bmp390_reg_modes_arr[i].starting_bit_pos;
            uint8_t mode_value = (reg_value & mask) >> bmp390_reg_modes_arr[i].starting_bit_pos;
...
esp_err_t bmp390_hwl_get_chip_id(device_interface_t dev_iface, uint8_t *read_data) {
    ...
    *read_data = 0;
    ...
    if((dev_iface.read_bytes(dev_iface.master_cfg, dev_iface.dev_cfg, BMP390_CHIP_ID_RO_REG, read_data, 1, dev_iface.iface_sel) != ESP_OK) || (bmp390_hwl_err(dev_iface) != ESP_OK)) {
        ESP_LOGE(bmp390_hwl_tag, "{Function <%s> in line %d}: Read bytes --> FAILED", __func__, __LINE__);
        return ESP_FAIL;
    }
...
esp_err_t bmp390_hwl_get_rev_id(device_interface_t dev_iface, uint8_t *read_data) {
    ...
    *read_data = 0;
    ...
    if((dev_iface.read_bytes(dev_iface.master_cfg, dev_iface.dev_cfg, BMP390_REV_ID_RO_REG, read_data, 1, dev_iface.iface_sel) != ESP_OK) || (bmp390_hwl_err(dev_iface) != ESP_OK)) {
        ESP_LOGE(bmp390_hwl_tag, "{Function <%s> in line %d}: Read bytes --> FAILED", __func__, __LINE__);
        return ESP_FAIL;
    }
...
esp_err_t bmp390_hwl_err(device_interface_t dev_iface) {
    ...
    if(dev_iface.read_bytes(dev_iface.master_cfg, dev_iface.dev_cfg, BMP390_ERR_REG_RO_REG, &err_reg, 1, dev_iface.iface_sel) != ESP_OK) {
        ESP_LOGE(bmp390_hwl_tag, "{Function <%s> in line %d}: Read bytes --> FAILED", __func__, __LINE__);
        return ESP_FAIL;
    }
...
esp_err_t bmp390_hwl_cmd_rdy_status(device_interface_t dev_iface) {
    ...
    if((dev_iface.read_bytes(dev_iface.master_cfg, dev_iface.dev_cfg, BMP390_STATUS_RO_REG, &status, 1, dev_iface.iface_sel) != ESP_OK) || (bmp390_hwl_err(dev_iface) != ESP_OK)) {
        ESP_LOGE(bmp390_hwl_tag, "{Function <%s> in line %d}: Read bytes --> FAILED", __func__, __LINE__);
        return ESP_FAIL;
    }
...
int bmp390_hwl_detect_soft_reset(device_interface_t dev_iface) {
    ...
    if((dev_iface.read_bytes(dev_iface.master_cfg, dev_iface.dev_cfg, BMP390_EVENT_RO_REG, &event, 1, dev_iface.iface_sel) != ESP_OK) || (bmp390_hwl_err(dev_iface) != ESP_OK)) {
        ESP_LOGE(bmp390_hwl_tag, "{Function <%s> in line %d}: Read bytes --> FAILED", __func__, __LINE__);
        return ESP_FAIL;
    }
...
esp_err_t bmp390_hwl_spi_en(device_interface_t dev_iface, bmp390_if_conf_reg_spi_t spi_mode) {
    ...
    if(dev_iface.iface_sel == NONE) {
        ESP_LOGE(bmp390_hwl_tag, "{Function <%s> in line %d}: No Interface was selected", __func__, __LINE__);
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if(dev_iface.iface_sel == SPI) {
        ...
        if((dev_iface.read_bytes(dev_iface.master_cfg, dev_iface.dev_cfg, BMP390_IF_CONF_RW_REG, &if_conf, 1, dev_iface.iface_sel) != ESP_OK) || (bmp390_hwl_err(dev_iface) != ESP_OK)) {
            ESP_LOGE(bmp390_hwl_tag, "{Function <%s> in line %d}: Read bytes --> FAILED", __func__, __LINE__);
            return ESP_FAIL;
        }
        ...
        bmp390_hwl_get_mode_val(dev_iface, BMP390_IF_CONF_RW_REG, BMP390_IF_CONF_SPI3_BIT);
        return ESP_OK;
    }

    /* SPI interface was not selected */
    ESP_LOGE(bmp390_hwl_tag, "{Function <%s> in line %d}: SPI interface must be selected in order to use this function", __func__, __LINE__);
    return ESP_ERR_INVALID_ARG;
}
...
esp_err_t bmp390_hwl_i2c_en_wdt(device_interface_t dev_iface, bmp390_if_conf_reg_i2c_wdt_tout_t i2c_wdt_tout) {
    ...
    if(dev_iface.iface_sel == I2C) {
        ...
        bmp390_hwl_get_mode_val(dev_iface, BMP390_IF_CONF_RW_REG, BMP390_IF_CONF_I2C_WDT_EN_BIT);
        bmp390_hwl_get_mode_val(dev_iface, BMP390_IF_CONF_RW_REG, BMP390_IF_CONF_I2C_WDT_SEL_BIT);
        return ESP_OK;
    }

    /* I2C interface was not selected */
    ESP_LOGE(bmp390_hwl_tag, "{Function <%s> in line %d}: I2C interface must be selected in order to use this function", __func__, __LINE__);
    return ESP_ERR_INVALID_ARG;
}
...
esp_err_t bmp390_hwl_set_pwr_mode(device_interface_t dev_iface, bmp390_pwr_ctrl_mode_t pwr_mode) {
    ...
    if((dev_iface.read_bytes(dev_iface.master_cfg, dev_iface.dev_cfg, BMP390_PWR_CTRL_RW_REG, &pwr_ctrl, 1, dev_iface.iface_sel) != ESP_OK) || (bmp390_hwl_err(dev_iface) != ESP_OK)) {
        ESP_LOGE(bmp390_hwl_tag, "{Function <%s> in line %d}: Read bytes --> FAILED", __func__, __LINE__);
        return ESP_FAIL;
    }
    ...
    bmp390_hwl_get_mode_val(dev_iface, BMP390_PWR_CTRL_RW_REG, BMP390_PWR_CTRL_MODE_BITS);
    return ESP_OK; 
}
...
esp_err_t bmp390_hwl_press_en(device_interface_t dev_iface) {
    ...
    bmp390_hwl_get_mode_val(dev_iface, BMP390_PWR_CTRL_RW_REG, BMP390_PWR_CTRL_PRESS_EN_BIT);
    return ESP_OK;
}
...
esp_err_t bmp390_hwl_temp_en(device_interface_t dev_iface) {
    ...
    /* Write mask to PWR_CTRL register and check for errors */
    if((dev_iface.write_bytes(dev_iface.master_cfg, dev_iface.dev_cfg, BMP390_PWR_CTRL_RW_REG, mask, dev_iface.iface_sel) != ESP_OK) || (bmp390_hwl_err(dev_iface) != ESP_OK)) {
        ESP_LOGE(bmp390_hwl_tag, "{Function <%s> in line %d}: Enable temperature sensor --> FAILED", __func__, __LINE__);
        return ESP_FAIL;
    }

    bmp390_hwl_get_mode_val(dev_iface, BMP390_PWR_CTRL_RW_REG, BMP390_PWR_CTRL_TEMP_EN_BIT);
    return ESP_OK;
}
...
esp_err_t bmp390_hwl_set_osr_press(device_interface_t dev_iface, bmp390_osr_press_t osr_press) {
    ...
    if((dev_iface.read_bytes(dev_iface.master_cfg, dev_iface.dev_cfg, BMP390_OSR_RW_REG, &osr, 1, dev_iface.iface_sel) != ESP_OK) || (bmp390_hwl_err(dev_iface) != ESP_OK)) {
        ESP_LOGE(bmp390_hwl_tag, "{Function <%s> in line %d}: Read bytes --> FAILED", __func__, __LINE__);
        return ESP_FAIL;
    }
    ...
    bmp390_hwl_get_mode_val(dev_iface, BMP390_OSR_RW_REG, BMP390_OSR_P_BITS);
    return ESP_OK;
}
...
esp_err_t bmp390_hwl_set_osr_temp(device_interface_t dev_iface, bmp390_osr_temp_t osr_temp) {
    ...
    bmp390_hwl_get_mode_val(dev_iface, BMP390_OSR_RW_REG, BMP390_OSR_T_BITS);
    return ESP_OK;
}
...
esp_err_t bmp390_hwl_set_odr(device_interface_t dev_iface, bmp390_odr_sel_t odr_sel) {
    ...
    if((dev_iface.read_bytes(dev_iface.master_cfg, dev_iface.dev_cfg, BMP390_ODR_RW_REG, &odr, 1, dev_iface.iface_sel) != ESP_OK) || (bmp390_hwl_err(dev_iface) != ESP_OK)) {
        ESP_LOGE(bmp390_hwl_tag, "{Function <%s> in line %d}: Read bytes --> FAILED", __func__, __LINE__);
        return ESP_FAIL;
    }
    ...
    bmp390_hwl_get_mode_val(dev_iface, BMP390_ODR_RW_REG, BMP390_ODR_ODR_SEL_BITS);
    return ESP_OK;
}
...
esp_err_t bmp390_hwl_set_iir_coef(device_interface_t dev_iface, bmp390_config_coef_t iir_coef) {
    ...
    if((dev_iface.read_bytes(dev_iface.master_cfg, dev_iface.dev_cfg, BMP390_CONFIG_RW_REG, &config, 1, dev_iface.iface_sel) != ESP_OK) || (bmp390_hwl_err(dev_iface) != ESP_OK)) {
        ESP_LOGE(bmp390_hwl_tag, "{Function <%s> in line %d}: Read bytes --> FAILED", __func__, __LINE__);
        return ESP_FAIL;
    }
    ...
    bmp390_hwl_get_mode_val(dev_iface, BMP390_CONFIG_RW_REG, BMP390_CONFIG_IIR_BITS);
    return ESP_OK;
}
...
esp_err_t bmp390_hwl_exec_cmd(device_interface_t dev_iface, bmp390_cmd_t cmd_sel) {
    ...
    if((dev_iface.read_bytes(dev_iface.master_cfg, dev_iface.dev_cfg, BMP390_CMD_RW_REG, &cmd, 1, dev_iface.iface_sel) != ESP_OK) || (bmp390_hwl_err(dev_iface) != ESP_OK)) {
        ESP_LOGE(bmp390_hwl_tag, "{Function <%s> in line %d}: Read bytes --> FAILED", __func__, __LINE__);
        return ESP_FAIL;
    }
...
esp_err_t bmp390_hwl_read_raw_data(device_interface_t dev_iface, uint32_t *adc_temp, uint32_t *adc_press) {
    ...
    *adc_press = 0;
    ...
    /* Read DATA register content and check for errors */
    if((dev_iface.read_bytes(dev_iface.master_cfg, dev_iface.dev_cfg, BMP390_DATA_0_RO_REG, data, 6, dev_iface.iface_sel) != ESP_OK) || (bmp390_hwl_err(dev_iface) != ESP_OK)) {
        ESP_LOGE(bmp390_hwl_tag, "{Function <%s> in line %d}: Read bytes --> FAILED", __func__, __LINE__);
        return ESP_FAIL;
    }
